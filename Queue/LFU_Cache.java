...
public class LFU_Cache {
    ...
    * @param cache: a hash map that has key to Node mapping, which is used for storing all nodes by their keys
    * @param frequencyMap: a hash map that has frequency to linked list mapping, which used for storing all double linked list by their frequencies
    */
     ...
     *  - sub condition 1: if LFU cache does NOT have enough space, remove the Least Recent Used node in minimum frequency list, then add new node
    ...
    */
    ...
    public void updateNode(DLLNode curNode) {
        ...
        // if current list is the list which has lowest frequency and current node is the only node in that list, we need to remove the entire list and then increase min frequency value by 1
        if (curFreq == minFrequency && curList.listSize == 0) {
            minFrequency++;
        }

        curNode.frequency++;
        // add current node to another list with key = current frequency + 1,
